...
const MyDonation = () => {
  ...
  if (isError)
    return <p className="text-center text-red-600 dark:text-red-400">Error loading donations.</p>;
  if (!donations.length)
    return <p className="text-center text-gray-700 dark:text-gray-300">No donations found.</p>;
      ...
      <div className="hidden md:block overflow-x-auto rounded-md border border-gray-300 dark:border-gray-700">
        <table className="w-full table-auto border-collapse border border-gray-300 dark:border-gray-700 rounded-md">
          <thead className="bg-gray-100 dark:bg-gray-800">
            <tr>
              {["S/N", "Pet Image", "Pet Name", "Donated Amount", "Actions"].map((h) => (
                <th key={h} className="py-2 px-4 text-left text-gray-700 dark:text-gray-300">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {donations.map((donation, index) => (
              ...
              >
                ...
                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                  ৳{typeof donation.amount === "number" ? donation.amount.toFixed(2) : "0.00"}
                </td>
                <td className="py-2 px-4">
                  <div className="flex gap-2 flex-wrap">
                    <Link to={`/donation-details/${donation.campaignId}`}>
                     <Button
                      size="sm"
                      variant="outline"
                    ...
                    >
                      {refundMutation.isPending ? "Processing..." : "Ask for Refund"}
                    </Button> */}
                  </div>
                </td>
              </tr>
          ...
          >
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Amount: ৳{typeof donation.amount === "number" ? donation.amount.toFixed(2) : "0.00"}
                </p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <Button
                ...
                variant="outline"
                className="border-[#34B7A7] text-[#34B7A7] hover:bg-[#34B7A7] hover:text-white"
                onClick={() => navigate(`/donation-details/${donation.campaignId}`)}
              >
                Details
              </Button>
              <Button
